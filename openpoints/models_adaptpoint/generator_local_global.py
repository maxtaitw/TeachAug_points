...
class Teachaug_Augmentor(nn.Module):
    def __init__(self, w_num_anchor=4, w_sigma=0.5, w_R_range=10, w_S_range=3, w_T_range=0.25, g_R_range=10, g_S_range=1.5, g_T_range=0.25):
        super().__init__()
        ...
        self.g_R_range = g_R_range
        self.g_S_range = g_S_range
        self.g_T_range = g_T_range
    ...
    def forward(self, xyz):
        ...
        # probs = self.predict_prob_layer(xyz, fps_idx)     # probs: [B, N, 3] 
        # probs = self.predict_prob_layer(xyz, fps_idx)     # [B, M, 3]
        # probs, offset = self.predict_prob_layer(xyz, fps_idx)     # [B, M, 3]
        # probs_local, probs_global = self.predict_prob_layer(xyz, fps_idx)
        probs_local, weight, masking = self.predict_prob_layer(xyz, fps_idx)
        # print('probs:', probs[0, :, :])

        # Local transformation at anchor point
        xyz_local_transformed = self.local_transformaton(xyz_normalize, probs_local)  # (B,M,N,3)
    ...
    def global_transformation(self, xyz_local_transformed, prob):
        # print(xyz_local_transformed.shape)
        B, _, _ = xyz_local_transformed.shape

        prob_R = prob[:,:,0:3]                        #   [B, 3]
        prob_R = torch.tanh(prob_R)                       #   [B, 3]
        # prob_R = F.tanh(prob_R)                       #   [B, 3]
        prob_R = prob_R * self.g_R_range              #   [B, 3]
        degree = torch.tensor(math.pi).to(xyz_local_transformed.device) * prob_R.to(xyz_local_transformed.device) / 180.0  #   [B, 3], sampling from (-R_range, R_range)

        prob_S = prob[:, :, 3:6]                      #  [B, 3]
        prob_S = torch.tanh(prob_S)                    #  [B, 3]
        # prob_S = F.sigmoid(prob_S)                    #  [B, 3]
        prob_S = prob_S * (self.g_S_range-1) + 1      #  [B, 3]
        scale = prob_S.to(xyz_local_transformed.device) # [B, 3], sampling from (1, S_range)

        scale = scale + 1 * (scale == 0)  # Scaling factor must be larger than 1

        # prob_T = prob[:, :, 6:9]                    #   [B, 3]
        # prob_T = torch.tanh(prob_T)                     #   [B, 3]
        # # prob_T = F.tanh(prob_T)                     #   [B, 3]
        # prob_T = prob_T * self.g_T_range            #   [B, 3]
        # trl = prob_T.to(xyz_local_transformed.device)  # [B, 3], sampling from (1, S_range)


        # Scaling Matrix
        S = scale.unsqueeze(dim=-2) * torch.eye(3).to(xyz_local_transformed.device)  # scailing factor to diagonal matrix,3) ->,3,3)
        ...
        R = torch.cat([cz * cy, cz * sy * sx - sz * cx, cz * sy * cx + sz * sx,
                      sz * cy, sz * sy * sx + cz * cy, sz * sy * cx - cz * sx,
                      -sy, cy * sx, cy * cx], dim=-1).reshape(B, 3, 3)
        xyz_global_transformed = xyz_local_transformed @ R @ S.reshape(B, 3, 3) # + trl.reshape(B, 1, 3)
        # print(R.shape, S.shape)
        # print(xyz_global_transformed.shape)
        return xyz_global_transformed
...
class SAComponent(nn.Module):
    def __init__(self, in_channel=3, embed_dim=64, res_expansion=1.0,
        ...
        self.global_pooling = nn.MaxPool1d(2048)
        self.weight_head = nn.Sequential(
            # nn.Linear(64, 1),
            # nn.Sigmoid()
            nn.Conv1d(in_channels=3, out_channels=1, kernel_size=1, bias=False),
            # nn.BatchNorm1d(2),
            nn.Sigmoid()
        )
    ...
    def forward(self, x, a_index=None):
        ...
        # global_feat = self.global_layer(xyz_raw.permute(0, 2, 1)).permute(0, 2, 1)      #   [B, N, C]
        # # global_feat = torch.max(global_feat, dim=1, keepdim=True)[0]                    #   [B, 1, C]
        # weight = self.prob_head(global_feat)    #   [B, 1, 3*3]
        # print('0', len(x_list))
        # print('1', x_list[-1].shape) [32, 1024, 128]
        # print('11', x_list[0].shape) [32, 64, 2048]
        # print('2', xyz_raw.shape)
        # print('3', xyz.shape)
