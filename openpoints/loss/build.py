...
@LOSS.register_module()
class SmoothCrossEntropy_batch(torch.nn.Module):
    def __init__(self, label_smoothing=0.2, 
                 ignore_index=None, 
                 num_classes=None, 
                 weight=None, 
        ...
        super(SmoothCrossEntropy_batch, self).__init__()
        self.label_smoothing = label_smoothing
        self.ignore_index = ignore_index
        self.return_valid = return_valid
        # Reduce label values in the range of logit shape
    ...
    def forward(self, pred, gt):
        ...
        if self.label_smoothing > 0:
            ...
            log_prb = F.log_softmax(pred, dim=1)
            if self.weight is not None:
                loss = -(one_hot * log_prb * self.weight).sum(dim=1)
            else:
                loss = -(one_hot * log_prb).sum(dim=1)
        else:
            loss = F.cross_entropy(pred, gt, weight=self.weight, reduce=False)
...
@LOSS.register_module()
class WeightedSmoothCrossEntropy(torch.nn.Module):
    def __init__(self, label_smoothing=0.2, 
                 ignore_index=None, 
                 num_classes=None, 
                 weight=None, 
        ...
        super(WeightedSmoothCrossEntropy, self).__init__()
        self.label_smoothing = label_smoothing
        self.ignore_index = ignore_index
        self.return_valid = return_valid
        # Reduce label values in the range of logit shape
    ...
    def forward(self, pred, gt, sample_weight):
        if len(pred.shape)>2:
            pred = pred.transpose(1, 2).reshape(-1, pred.shape[1])
        gt = gt.contiguous().view(-1)
