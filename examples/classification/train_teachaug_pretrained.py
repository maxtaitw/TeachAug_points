...
from openpoints.function_adaptpoint import Form_dataset_cls, get_feedback_loss_ver1, get_feedback_loss_teacher, get_feedback_loss_teacher_v2
...
def main(gpu, cfg, profile=False):
    ...
    ######################################################
    # pretrained teacher
    if cfg.pretrained_teacher == True:
        logging.info('loading pretrained teacher model ...')
        teacher_model = torch.load(cfg.pretrained_teacher_path, map_location='cpu')
    # EMA Teacher
    else:
    ...
    for epoch in range(cfg.start_epoch, cfg.epochs + 1):
        ...
        if epoch > cfg.get('adaptpoint_adjustepoch', 0):
            fake_dataset = train_gan(cfg, gan_model_dict, train_loader, summary, writer, epoch, model, teacher_model)
...
def train_one_epoch(model, teacher_model, train_loader, optimizer, scheduler, epoch, cfg):
    loss_meter = AverageMeter()
    cm = ConfusionMatrix(num_classes=cfg.num_classes)
    npoints = cfg.num_points

    model.train()  # set model to training mode
    if cfg.pretrained_teacher == False:
        teacher_model.train()
